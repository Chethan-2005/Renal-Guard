...
class DoctorDashboardActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Doctor email from intent (only once)
        ...
        bottomNav.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.nav_profile -> {
                    // 🔹 Changed to open ProfileActivity
    ...
    // Reload patients every time activity resumes (e.g., returning from another page)
    override fun onResume() {
        super.onResume()
        if (doctorEmail.isNotEmpty()) {
            // Clear search text and reload all patients
            searchBar.setText("")
            loadPatients()
        }
    }
    ...
    private fun viewPatientDetails(patient: PatientModel) {
        ...
        intent.putExtra("patient_email", patient.patientEmail) // 🔹 Added
        intent.putExtra("doctor_email", doctorEmail) // important for coming back
        startActivity(intent)
    }
